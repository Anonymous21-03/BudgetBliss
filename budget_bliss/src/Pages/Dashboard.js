...
function Dashboard () {
  ...
  const nonPaymentPredictions = predictions.filter(
    p =>
      p.Description &&
      !['payment', 'settle all balances'].includes(p.Description.toLowerCase())
  )
            ...
            <tbody>
              {displayedPredictions.map((prediction, index) => (
                <tr key={index}>
                  ...
                  <td>{formatINR(parseFloat(prediction['Total Cost']))}</td>
